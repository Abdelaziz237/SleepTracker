 ...
 fun formatNights(nights: List<SleepNight>, resources: Resources): Spanned {
    ...
    sb.apply {
        ...
        nights.forEach {
            ...
            append("\t${convertLongToDateString(it.start)}<br>")
            if (it.end != it.start) {
                append(resources.getString(R.string.end_time))
                append("\t${convertLongToDateString(it.end)}<br>")
                append(resources.getString(R.string.quality))
                append("\t${convertNumericQualityToString(it.quality, resources)}<br>")
                append(resources.getString(R.string.hours_slept))
                // Hours
                append("\t ${it.end.minus(it.start) / 1000 / 60 / 60}:")
                // Minutes
                append("${it.start.minus(it.start) / 1000 / 60}:")
                // Seconds
                append("${it.end.minus(it.start) / 1000}<br><br>")
            }
        }
    }
